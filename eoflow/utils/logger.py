...
class Logger:
    def __init__(self, sess, config):
        ...
        self.summary_ops = {}
        self.train_summary_writer = tf.summary.FileWriter(os.path.join(self.config.log_dir, "train"),
                                                          self.sess.graph)
        self.cval_summary_writer = tf.summary.FileWriter(os.path.join(self.config.log_dir, "cval"))
    ...
    def summarize(self, step, summarizer="train", scope="", summaries_dict=None):
        ...
        :return:
        """
        summary_writer = self.train_summary_writer if summarizer == "train" else self.cval_summary_writer \
            if summarizer == "cval" else self.test_summary_writer
